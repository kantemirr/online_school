...
CATALOG: list[dict] = [
    {
        ...
        "modules": [
            {"title": "Знакомство со Scratch", "lessons": [
                {"title": "Сцена и спрайты", "theory_md": THEORY["scr1_sprites"],
                 "assignments": [{"type": "quiz", "title": "Основы Scratch", "questions": [
                     ...
                     _q("Что такое костюм спрайта?", ["Его внешний вид", "Его имя", "Его скорость"], [0]),
                     _q("Где находится центр сцены?", ["x: 0, y: 0", "x: 100, y: 100", "В углу"], [0]),
                     _q("Какие элементы можно добавить на сцену?", ["Спрайты", "Фоны", "Числа подряд"], [0, 1],
                        kind=QuestionKind.MULTIPLE),
                 ]}]},
                {"title": "Движение и события", "theory_md": THEORY["scr1_move"],
                 "assignments": [{"type": "quiz", "title": "Движение", "questions": [
                     ...
                     _q("Что делает блок «перейти в x: y:»?", ["Телепортирует спрайт в точку", "Меняет костюм", "Запускает игру"], [0]),
                     _q("Зачем блок «если касается края — оттолкнуться»?", ["Чтобы спрайт не убежал со сцены", "Чтобы ускорить спрайт", "Чтобы поменять фон"], [0]),
                 ]}]},
            ]},
            {"title": "Делаем игру", "lessons": [
                {"title": "Цикл и условие", "theory_md": THEORY["scr1_loop"],
                 "assignments": []},
                {"title": "Проект: своя игра", "theory_md": THEORY["scr1_project"],
    ...
    },
    {
        ...
        "modules": [
            {"title": "Костюмы и анимация", "lessons": [
                {"title": "Смена костюмов", "theory_md": THEORY["scr2_costumes"],
                 "assignments": [{"type": "quiz", "title": "Анимация", "questions": [
                     ...
                     _q("Какой блок переключает картинку спрайта?", ["Следующий костюм", "Идти 10 шагов", "Играть звук"], [0]),
                     _q("Что будет, если убрать паузу между костюмами?", ["Костюмы замелькают слишком быстро", "Анимация остановится", "Спрайт исчезнет"], [0]),
                     _q("Что можно добавить в историю?", ["Диалоги", "Звук", "Случайные ошибки"], [0, 1],
                        kind=QuestionKind.MULTIPLE),
                 ]}]},
                {"title": "Диалоги и звук", "theory_md": THEORY["scr2_dialog"],
    ...
    },
    {
        ...
        "modules": [
            {"title": "Числа", "lessons": [
                {"title": "Максимум и минимум", "theory_md": THEORY["algo_max"],
                 "assignments": [
                     {"type": "code", "title": "Максимум из двух",
                      "tests": [_t("3 8\n", "8"), _t("10 2\n", "10", hidden=True)]},
                 ]},
                {"title": "Факториал", "theory_md": THEORY["algo_fact"],
                 "assignments": [
                     {"type": "code", "title": "Факториал",
                      "tests": [_t("5\n", "120"), _t("3\n", "6", hidden=True)]},
                 ]},
                {"title": "Простое число", "theory_md": THEORY["algo_prime"],
                 "assignments": [
                     {"type": "quiz", "title": "Простые числа", "questions": [
                         ...
                         _q("Является ли 1 простым числом?", ["Нет", "Да", "Иногда"], [0]),
                         _q("Какое самое маленькое простое число?", ["2", "1", "0"], [0]),
                         _q("Как проверить делимость n на d без остатка?", ["n % d == 0", "n / d == 0", "n // d == 0"], [0]),
            ...
            {"title": "Строки", "lessons": [
                {"title": "Подсчёт гласных", "theory_md": THEORY["algo_vowels"],
                 "assignments": [
